...
import {
  SearchIcon, LeafIcon, CameraIcon, FolderIcon, RefreshIcon, CheckCircleIcon,
  AlertTriangleIcon, AlertOctagonIcon, FlaskIcon, ChartIcon, TargetIcon, BugIcon,
  StarIcon, DropletIcon, SunIcon, PotIcon, PillIcon, SproutIcon, InfoIcon,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><LeafIcon className="w-6 h-6 text-green-600" /> Plant Disease Detection</h1>
            ...
            >
              {preview ? (
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-gray-600 font-medium flex items-center justify-center gap-1.5"><CameraIcon className="w-4 h-4" /> Click or drag & drop a leaf image here</p>
            ...
            <div className="lg:col-span-7 space-y-5">
              ...
              <div className={`rounded-2xl p-6 border-2 ${result.is_healthy ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
                <div className="flex items-center gap-4">
                  <span className={result.is_healthy ? 'text-green-600' : 'text-red-600'}>
                    {result.is_healthy ? <CheckCircleIcon className="w-12 h-12" /> : <AlertTriangleIcon className="w-12 h-12" />}
                  </span>
                  <div className="flex-1">
                    ...
                    <p className={`text-sm mt-1 font-medium flex items-center gap-1.5 ${result.is_healthy ? 'text-green-600' : 'text-red-600'}`}>
                      {result.is_healthy
                        ? <><SproutIcon className="w-4 h-4 shrink-0" /> Your plant appears to be healthy! Great job taking care of it.</>
                        : <><AlertOctagonIcon className="w-4 h-4 shrink-0" /> Disease detected. Check the details below for treatment guidance.</>}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border p-5">
                ...
                <p className="text-xs text-gray-400 mt-2 flex items-center gap-1.5">
                  {confidenceValue >= 90 ? <><TargetIcon className="w-3.5 h-3.5 shrink-0 text-green-500" /> High confidence — model is very sure about this prediction</> :
              ...
              {result.is_healthy && (
                <div className="bg-green-50 border border-green-200 rounded-2xl p-5">
                  <h3 className="font-semibold text-green-800 mb-2 flex items-center gap-1.5"><StarIcon className="w-4 h-4 text-yellow-500" /> Tips to Keep Your Plant Healthy</h3>
                  <ul className="text-sm text-green-700 space-y-1">
                    <li className="flex items-center gap-1.5"><DropletIcon className="w-3.5 h-3.5 shrink-0 text-blue-500" /> Water regularly but avoid overwatering</li>
                    <li className="flex items-center gap-1.5"><SunIcon className="w-3.5 h-3.5 shrink-0 text-yellow-500" /> Ensure adequate sunlight exposure</li>
                    <li className="flex items-center gap-1.5"><PotIcon className="w-3.5 h-3.5 shrink-0 text-amber-600" /> Use nutrient-rich soil and proper fertilizers</li>
                    <li className="flex items-center gap-1.5"><BugIcon className="w-3.5 h-3.5 shrink-0 text-orange-600" /> Monitor for pests and insects regularly</li>
                  </ul>
                </div>
              )}
            </div>
