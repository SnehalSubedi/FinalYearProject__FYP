...
export default function InsectPage() {
  ...
  const handleFileChange = (e) => {
    ...
    if (!allowed.includes(file.type)) { toast.error('Please upload a JPEG, PNG, or WebP image.'); return }
  ...
  const handleSubmit = async () => {
    ...
    setResult(null)
    try {
      const res = await api.post('/insect/predict', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
      setResult(res.data)
      toast.success('Insect detection complete!')
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Detection failed. Try again.')
        ...
        <div className={`grid gap-6 grid-cols-1 ${result ? 'lg:grid-cols-12' : 'max-w-2xl mx-auto'}`}>
          ...
          <div className={`card space-y-5 ${result ? 'lg:col-span-5' : ''}`}>
            <div
              ...
              className="border-2 border-dashed border-orange-300 rounded-xl p-6 text-center cursor-pointer hover:border-orange-500 hover:bg-orange-50/50 transition-all duration-200 min-h-[300px] flex items-center justify-center"
            >
              {preview ? (
                <img src={preview} alt="Insect preview" className="max-h-[360px] mx-auto rounded-lg object-contain shadow" />
              ) : (
